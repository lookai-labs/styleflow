...
def _load_makeup_table(path: Path = _MAKEUP_XLSX) -> dict:
    """makeup.xlsx → {퍼스널 컬러: [메이크업, ...]} (결과 캐시됨)."""
    global _makeup_table_cache
    if _makeup_table_cache is not None:
        return _makeup_table_cache

    df = pd.read_excel(path, sheet_name=0)
    df["퍼스널 컬러"] = df["퍼스널 컬러"].ffill()

    table: dict = {}
    for personal_color, group in df.groupby("퍼스널 컬러"):
        table[personal_color] = [x for x in group["메이크업"].tolist() if isinstance(x, str)]

    _makeup_table_cache = table
    return table


def recommend_makeup(personal_color: dict) -> dict:
    """
    diagnose.py의 personal_color 결과 → makeup.xlsx 추천 메이크업.
    ...
    for label in candidate_labels:
        ...
        for item in table.get(makeup_label, []):
            if item not in recommended:
                recommended.append(item)
...
def recommend(diagnosis: dict, gender: str) -> dict:
    ...
    makeup = recommend_makeup(diagnosis.get("personal_color") or {})
