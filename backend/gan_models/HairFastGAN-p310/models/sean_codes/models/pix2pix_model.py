...
def load_average_feature():
    ...
    for i in range(19):
        ...
        for style_code_path in average_category_list:
            t = torch.from_numpy(np.load(os.path.join(average_style_code_folder, str(i), style_code_path + '.npy')))
            input_style_dic[str(i)][style_code_path] = t.cuda() if torch.cuda.is_available() else t
...
SEAN_OPT = Namespace(name='CelebA-HQ_pretrained', gpu_ids=[], checkpoints_dir='pretrained_models/sean_checkpoints',
