...
화이트밸런스 정책
-----------------
- 얼굴형/삼정: gray_world WB 적용 (기본, --no-shape-wb로 끔). 조명 영향을 줄여 랜드마크 정확도 향상.
- 퍼스널컬러: model_bundle/inference_config.json의 "white_balance" 값(현재 "sclera")을
  predict.py가 그대로 읽어 매 이미지마다 적용. 학습 때 쓴 값과 항상 동일해야 하므로
  이 파일에서 값을 바꾸지 말고, 재학습 시 train.py --white-balance로만 바꿀 것.
"""
...
import mediapipe as mp  # noqa: E402  (after protobuf patch)
...
def _detect_face_landmarks(image_path: str | Path, apply_wb: bool = True):
    ...
    return img_bgr, result.multi_face_landmarks[0].landmark, w, h
