...
def simulate_hair(request):
    ...
    try:
        # from .hairgan_service import run_hair
        from .hairgan_service import run_hair
    ...
    for i, item in enumerate(gan_results):
        ...
        results.append({'id': f'h{i + 1}', 'image': url, 'name': item['name']})
...
def simulate_makeup(request):
    ...
    try:
        from .beautygan_service import run_makeup
        output_dir = os.path.join(settings.MEDIA_ROOT, 'simulations')
